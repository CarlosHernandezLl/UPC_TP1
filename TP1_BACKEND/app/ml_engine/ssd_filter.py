...
class SSDService:
    ...
    def extraer_todos_los_minutos_estacionarios(cls, df_raw: pd.DataFrame) -> pd.DataFrame:
        """
        Filtra la serie temporal continua extraída de la BD y extrae 
        únicamente los bloques en equilibrio térmico (Data de Oro).
        """
        if df_raw.empty:
            return pd.DataFrame()
